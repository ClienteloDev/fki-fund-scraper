...
def main() -> int:
    ...
    print("FIELD")
    print("-" * 62)
    print(f"  {'field':<26}{'valid':>8}{'suspic.':>9}{'confl.':>8}{'missing':>9}")
    ...
    for field in ALL_AUDITED_FIELDS:
        ...
        print(
            f"  {field:<26}"
            f"{counts.get('valid', 0):>8}"
            f"{counts.get('suspicious', 0):>9}"
            f"{counts.get('conflicting', 0):>8}"
    ...
    if args.examples > 0 and report.findings:
        ...
        for finding in report.findings:
            ...
            print(f"      value:  {finding.normalized_value or finding.extracted_value}")
